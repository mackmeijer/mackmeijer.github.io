        ...
        if (in_array($fileActualExt, $allowed)){
            ...
            if ($fileError === 0) {
                ...
                if ($fileSize < 1000000) {
                    ...
                    $stmt = $db_connection->prepare('SELECT * FROM photo_2 WHERE id= :id');
                    $stmt->execute(['id' => $_SESSION['user_id']]);
                    $row = $stmt->fetch(PDO::FETCH_ASSOC);   
                    if( ! $row) {
                        $stmt = $db_connection->prepare('INSERT INTO photo_2 SET  file_name = :file_name, id = :id');
                        $stmt->execute(['id' => $_SESSION['user_id'], 'file_name' => $fileNameNew]);  
                    }

                    else {
                    $stmt = $db_connection->prepare('UPDATE photo_2 SET  file_name = :file_name WHERE id = :id');
                    $stmt->execute(['id' => $_SESSION['user_id'], 'file_name' => $fileNameNew]);  
                    }
